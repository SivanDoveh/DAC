...
import sys
# sys.path.append('/dccstor/paolac1/FT_CLIP/LEONID_BRANCH/FT_CLIP/lib/')
...
def get_retrieval_scores_batched(model, joint_loader, device="cuda"):
    ...
    for batch in tqdm_loader:
        ...
        for c_option in batch["caption_options"]:
            ...
            caption_embeddings = model(None,caption_tokenized.to(device)).cpu().numpy() # B x D
...
if __name__ == "__main__":
    ...
    model.eval()
    root_root_dir = '/dccstor/sivandov1/data/evlk/aro/'
    root_dir = f"{root_root_dir}datasets"
    vgr_dataset = VG_Relation(image_preprocess=image_preprocess, download=True, root_dir=root_dir)
    ...
    vga_dataset = VG_Attribution(image_preprocess=image_preprocess, download=True, root_dir=root_dir)
    ...
    coco_order_dataset = COCO_Order(image_preprocess=image_preprocess, download=True, root_dir=root_dir, split='test')
    ...
    flickr_order_dataset = Flickr30k_Order(image_preprocess=image_preprocess, split='test', root_dir=root_dir)
    ...
    with open(res_file, "a") as f:
        writer = csv.writer(f)
        writer.writerow([args.resume, vg_relation_macro_acc, vg_attribution_macro_acc, coco_order, flickr30k_order])