...
class Expander(Dataset):
    def __init__(
        ...
        self.args = args
        lists = []
        if self.args.use_only_quality_captions and (
            self.args.use_v2_extra_blip_expanders
            or self.args.use_v2_extra_blip_expanders_noun
            or self.args.use_v2_extra_blip_expanders_adj
        ):
            images_with_text = os.listdir(args.quality_captions_folder)
        ...
        elif self.args.use_only_quality_captions and self.args.use_v2_extra_blip_expanders:
            list1 = os.listdir(args.quality_captions_folder)
            list2 = os.listdir(args.v2_path_extra_blip_cap_expanders_sen)
            images_with_text = list(set(list1).intersection(list2))
        elif self.args.use_only_quality_captions:
            images_with_text = os.listdir(args.quality_captions_folder)
    ...
    def __getitem__(self, idx):
        ...
        self.captions[idx] = self.use_extra.create_pos(
            img_path.split("/")[-1], self.captions[idx], raw
        )
...
def choose_feat_mil(
    args, image_features, text_features, logit_scale, list_amount_of_pos
):
    ...
    for img_ind in range(images_num):
        ...
        if args.mil_co_loader_type == "max":
            ...
            mil_text_features_per_image.append(
                text_feature_divided_list[img_ind][:, max_idx]
            )
