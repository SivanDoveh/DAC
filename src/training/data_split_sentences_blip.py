...
class TextDataset(Dataset):
    def __init__(self, args):
        ...
        self.file_paths = [os.path.join(folder_path, f) for f in os.listdir(folder_path)]# if "good_mapping" in f]
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"The file {example_num} doesn't have image or caption does not have expander")
            return []
        ...
        with open(f'{self.args.save_renamed_text_expanders}/{image_name}.txt', 'w') as f:
...
def bla():
    ...
    for img_name in images_names:
        print(img_name)
        f = open(f'{os.path.join("/dccstor/sivandov1/data/blip2_positives_cc3m/", img_name)}.json')
        caption = json.load(f)
