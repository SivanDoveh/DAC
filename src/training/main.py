...
from vl_checklist.merge_evaluate_vl import EvaluateAllVL_merged as EvaluateAllVL_merged
...
def main():
    ...
    if args.blip_cap or args.noun_pos or args.use_extra_cc3m_expanders or args.use_extra_blip_cap_expanders or (args.use_v2_extra_blip_expanders and not args.use_expanders_as_additional_data and not args.mil_co_loader and not args.mil_gpt) or args.calc_pos_sim or args.avg_pos_features:#evlk
        args.vl_pos = True
    ...
    if args.radar:
        generate_chart(args,args.vl_checklist_accuracy_jsons_folder, 'corpus.json', 'itc', chart_type='radar', models=args.eval_radar,name = args.radar_name,ep_eval=args.eval_radar_ep,radar_legends=args.radar_legends,start_radar = args.start_radar)
        return
    ...
    if args.save_eval_model:
        ...
        print(f'saved in {os.path.join(args.checkpoint_path, f"eval_epoch_{start_epoch}.pt")}')
        return
    if 'train' not in data or args.eval_only:
        if args.eval_merged_vl_checklist:
            EvaluateAllVL_merged(preprocess_val, start_epoch, args, writer)
        elif args.eval_vl_cklist:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
            if args.eval_vl_cklist:
                EvaluateAllVL(model, preprocess_val, completed_epoch, args, writer)
            if args.eval_recall and not args.ZS_steps_eval:
