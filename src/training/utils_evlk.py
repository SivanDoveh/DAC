...
def average_pos_feat(text_features,list_amount_of_pos,match_list):
    ...
    for num_of_sentences,match_list_item in zip(list_amount_of_pos,match_list):
        list_of_avg_pos.append(sum(match_list_item.to(device, non_blocking=True,dtype=torch.float32) * pos_feats[last_index:last_index + num_of_sentences])/num_of_sentences)
