...
def EvaluateAllVL_merged(preprocess_val, start_epoch, args, writer):
    ...
    if args.eval_vl_cklist_all:
        vl_eval = Evaluate(config_file="vl_checklist/configs/clip_all_obj.yaml", model1=model1,model2=model2,
                           preprocess_val=preprocess_val, epoch=start_epoch, args=args, tb_writer=writer)
        vl_eval.start()

    vl_eval = Evaluate(config_file="vl_checklist/configs/clip_all_attribute.yaml", model1=model1,model2=model2,
                       preprocess_val=preprocess_val, epoch=start_epoch, args=args, tb_writer=writer)
    vl_eval.start()

    vl_eval = Evaluate(config_file="vl_checklist/configs/clip_all_rel.yaml", model1=model1,model2=model2,
                       preprocess_val=preprocess_val, epoch=start_epoch, args=args, tb_writer=writer)
    vl_eval.start()

    vl_eval = Evaluate(config_file="vl_checklist/configs/clip_all_rel_spatial.yaml", model1=model1,model2=model2,
                       preprocess_val=preprocess_val, epoch=start_epoch, args=args, tb_writer=writer)
    vl_eval.start()


class Evaluate(object):
    def __init__(self, config_file, model1,model2,preprocess_val,epoch,args,tb_writer=None) -> None:
    ...
    def clip_model_wrapper(self, images, texts):
        ...
        for i, chunk_i in enumerate(chunks(images, self.batch_size)):
            for j in range(len(chunk_i)):
                ...
                with torch.no_grad():
                    image_features1, text_features1, logit_scale1 = self.model1(image, text)
                    image_features2, text_features2, logit_scale2 = self.model2(image, text)
                    logits_per_image1 = logit_scale1 * image_features1 @ text_features1.t()
                    logits_per_image2 = logit_scale2 * image_features2 @ text_features2.t()
                    logits_per_image = (self.alpha*logits_per_image1 + (1-self.alpha)*logits_per_image2)
    ...
    def eval(self, data_type):
        ...
        if self.task == 'itc':
            for name in d.data:
                ...
                path = os.path.join(self.args.logs.split('//')[0].split('src')[0], 'eval_jsons_correct',
                                             self.dir.split('/')[1], 'merged_models_'+str(self.alpha))
